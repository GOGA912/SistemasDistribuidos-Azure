...
public class Servidor {
    ...
    public static void main(String[] args) {
        ...
        try(ServerSocket ss = new ServerSocket(3000)) {
            ...
            while (true) {
                Socket cl = ss.accept();
                System.out.println("Cliente conectado desde "+cl.getPort());
    ...
    private static class MClientes implements Runnable{
        ...
        public void run(){
            try {
                ...
                System.out.println("Cliente desconectado inesperadamente desde "+cl.getPort());
            } finally {
                try {
                    cl.close();
                    System.out.println("Cliente desconectado desde "+cl.getPort());
                } catch (IOException e) {
    ...
    private static void MOperaciones(BufferedReader ent,PrintWriter sal, CuentaBancaria cuenta) throws IOException{
        ...
        while (true) {
            ...
            switch (operacion) {
                case "C":
                    sal.println("Saldo actual: $" + cuenta.consultarSaldo());
                    break;
                case "D":
                    double montoD = Double.parseDouble(ent.readLine());
                    cuenta.depositar(montoD);
                    sal.println("Depósito exitoso. Nuevo saldo: $" + cuenta.consultarSaldo());
                    break;
                case "R":
                ...
                case "T":
                    ...
                    if (cuentas.containsKey(cuentaDestino)) {
                        ...
                        if (cuenta.transferir(destino, montoT)) {
                            ...
                            sal.println("Saldo insuficiente o monto inválido.");
                        }
                    } else {
